...
\end{itemize}
\noindent\textbf{Example (Group by Departments)}

\begin{verbatim}
    df.groupby(["Department"])["Salary"].mean()
\end{verbatim}
\begin{itemize}
    \item The above line can be read as: group by department, and take the mean of each department's salary.
    \item More generally, groupby takes on the following form:
\end{itemize}
\begin{verbatim}
    df.groupby([group_var])[value_var].agg_func()
\end{verbatim}

\noindent\textbf{Example (Pandas Merge with Duplicate Keys)}
\begin{verbatim}
...
\section{Sampling}
\begin{itemize}
    \item Given an RV $X$, $\text{Var}(X) = \mathbb{E}[(X - \mathbb{E}[X])^2] = \mathbb{E}[X^2] - \mathbb{E}[X]^2$
    ...
    \item \textbf{Stratified samples} - point estimate is a weighted average of sub populations
        \begin{itemize}
            ...
            \item $\text{Var}(\bar{X}) = \text{Var}(w_1\bar{X}_1) + \ldots + \text{Var}(w_n\bar{X}_n) = w_1^2\text{Var}(\bar{X}_1) + \ldots + w_n^2\text{Var}(\bar{X}_n)$
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{1-D OLS}:
        ...
        \]
        \[
            \rho = \frac{\text{Cov}(X, Y)}{\sqrt{\text{Var}(X)\text{Var}(Y)}} = \text{Cov}(X, Y) \quad X, Y \text{ standardized}
        \]
        \[
...
\begin{itemize}
    ...
    \item A large $\beta_j$ only indicates that one predictor is important in context of other predictors. To compare $\beta_i, \beta_j$, $X_i, X_j$ must be standardized.
...
\[
    \arg\min_\beta |Y - X\beta|^2 + \alpha|\beta|^2 \Rightarrow \hat{\beta} = (X^TX + \alpha I)^{-1}X^Ty \Rightarrow \hat{y} = X(X^TX + \alpha I)^{-1}y
\]

\begin{itemize}
    \item Trades off efficiency (how much the model relies on the data) for robustness (how generalizable the data is)
        \begin{itemize}
            \item Trades off a small increase in $\text{bias}^2 = (\mathbb{E}[\hat{w}] - w)^2$ for a large decrease in $\text{Var}(\hat{w})$
        \end{itemize}
\end{itemize}
...
\noindent\textbf{Example (Sales)} \\
\\
A linear regression predictor has been fit on data that records sales in the given month and year. \\
\\
What are the potential issues in the model fitting pipeline below? \\
\\
\begin{verbatim}
    X = df[["month", "year"]]
    y = df[["sales"]]
...
Issues:
\begin{itemize}
    \item Model does not account for autoregressive nature of data. Given that the predictors are w.r.t time,
    the best predictor for this month's sales may actually be last month's sales.
    \item The predictors are not standardized, meaning that we cannot compare the magnitude of coefficients directly. Thus,
    we cannot isolate which predictor in the model is most significant.
\end{itemize}

A revised implementation would be:

\begin{verbatim}
    # Create the AR(1) column
    df["prev_month_sales"] = df["sales"].shift(1)
    df.dropna(inplace=True)

    X = df[["month", "year", "prev_month_sales"]]
    y = df[["sales"]]
    ...
    # Use tscv.split since one of our predictors are time-dependent
    tscv = TimeSeriesSplit()
    for train_idx, test_idx in tscv.split(X):
        X_train, X_test = X[train_idx], X[test_idx]
        y_train, y_test = y[train_idx], y[test_idx]
...
\[
    z^* = \frac{\bar{x}_A - \bar{x}_B - 0}{\sqrt{\frac{\sigma^2_A}{n_A} + \frac{\sigma^2_B}{n_B}}}
\]
...
\begin{verbatim}
    ...
    X_norm = StandardScalar().fit_transform(X)
\end{verbatim}

\subsection{Label Encoding, One-Hot Encoding}
...
\begin{itemize}
    \item Assigns each categorical variable outcome a numerical level.
    \item As such, label encoding assumes that there is an ordering in outcomes, when none may exist. This may
    make interpretation difficult, and may result in inflated coefficients.
\end{itemize}

\noindent\textbf{One-Hot Encoding}
\begin{verbatim}
...
\begin{itemize}
    \item Assigns each categorical variable outcome a separate column (or indicator variable).
    \item Let $l$ be an arbitrary outcome. If $\mathbf {1}_{\{x_i \in l\}}$, then for the column
    associated with $l$, $x_i$'s value is 1.
    \item Unlike label encoding, one-hot encoding does not assume an ordering in outcomes.
    \item However, too many outcomes can lead to a significant increase in predictors, which can distill possible
    association and pull coefficients towards 0.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Encoder/Autoencoder} - reduce $X_1 \ldots X_n$ to dense representation (embedding vector), then upsample the response.
    \item Embeddings can be stored in vector database. Pre-computed embeddings by a well-trained model can enhance the response of other LLMs that query the vector database. The vector
    database can also serve as an embedding cache.
\end{itemize}
