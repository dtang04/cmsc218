...
\begin{itemize}
    \item Trades off efficiency (how much the model relies on the data) for robustness (how generalizable the data is)
        \begin{itemize}
            \item Trades off a small increase in $\text{bias}^2 = (\mathcal{E}[\hat{w}] - w)^2$ for a large decrease in $\text{Var}(\hat{w})$
        \end{itemize}
\end{itemize}
...
\begin{verbatim}
    ...
    encoded_X_cols = OneHotEncoder().fit_trnasform(X[cat_col])
\end{verbatim}
